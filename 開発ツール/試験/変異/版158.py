...
CASES = [
  ...
  ("★同じ管理番号の行を1つにまとめない（1ファイル＝1予定に戻る）",
   ...
   u'''    const cur = null; best.set(k + ":" + Math.random(), { row: r, n: 1 }); return;
    cur.n++;''', "smoke_v158_main.js"),
  ("★代表を「いちばん新しい1件」にしない（先に並んだ行のまま）",
   u'''    if(lgRowWhenMs(r) > lgRowWhenMs(cur.row)) cur.row = r;''', u'''    void 0;''', "smoke_v158_main.js"),
  ("重なった数を数えない", u'''    cur.n++;
    if(lgRowWhenMs(r) > lgRowWhenMs(cur.row)) cur.row = r;''',
   u'''    if(lgRowWhenMs(r) > lgRowWhenMs(cur.row)) cur.row = r;''', "smoke_v158_main.js"),
  ("月のマスに ×N を出さない", u'''          + (ev.dupN > 1 ? '<b class="cal-dup-n">×' + ev.dupN + '</b>' : '') + '</span>';''',
   u'''          + '</span>';''', "smoke_v158_main.js"),
  ("今後の予定に ×N を出さない",
   u'''      +   (ev.dupN > 1 ? ' <span class="cu-dup" title="同じ管理番号のファイルが ' + ev.dupN + ' 個あります。🧹 重複の整理 で1つにできます">×' + ev.dupN + ' 重複</span>' : '') + '</td>\'''',
   u'''      + '</td>\'''', "smoke_v158_main.js"),
  ("予定の説明に、重なっていることを書かない",
   u'''  const dLine = (ev.dupN > 1)''', u'''  const dLine = (false)''', "smoke_v158_main.js"),
  ("★予定を動かす相手が先頭の行のまま",
  ...
  return best;''', "smoke_v158_main.js"),
]
...
for name, old, new, test in CASES:
    ...
    n_run += 1
    base = io.open(MAIN, encoding="utf-8", newline="").read()
    o, n = CR(old), CR(new)
    if base.count(o) != 1:
