...
CASES = [
  ...
  ("★工事フォルダを掴めなくても、戸別ごとに聞き直す",
   u'''  if(inImport && _genbaNoDir) return noDirNote;   // 1件目で掴めなかった＝残りも同じ。何度も窓を出さない''',
   u'''  void 0;''', "smoke_v159_main.js", "main"),
  ("★取り込み以外の書き戻しまで、前の取り込みの印で素通りする",
   u'''  if(inImport && _genbaNoDir) return noDirNote;   // 1件目で掴めなかった＝残りも同じ。何度も窓を出さない''',
   u'''  if(_genbaNoDir) return noDirNote;''', "smoke_v159_main.js", "main"),
  ("★次の取り込みで、フォルダを掴みに行かない（印を戻さない）",
   u'''    _genbaNoDir = false;      // 取り込みごとに数え直す（前回フォルダを掴めなくても、今回は試す）''',
   u'''    void 0;''', "smoke_v159_main.js", "main"),
  ("枠に付けていた名前を、写真が戻ったときに捨てる",
   u'''        if(back && !String(back.label || "").trim()) back.label = x.label;''',
   u'''        void back;''', "smoke_v159_main.js", "main"),
  ...
  ("★工事フォルダを掴めていないのに、取り込みで知らせない",
   u'''        if(inImport && note && /工事フォルダ未リンク/.test(note)){ _genbaNoDir = true; return noDirNote; }''',
   u'''        void 0;''', "smoke_v159_main.js", "main"),
