...
CASES = [
  ...
  (u"★まとめて選んでも一覧に出ない（最初の1つだけ開く）",
   u'''  if(fs.length === 1){ await openJsonFile(fs[0]); return; }   // 1つだけ＝そのまま開く''',
   u'''  { await openJsonFile(fs[0]); return; }''',
   "smoke_v164_genba.js", "genba"),
  (u"★管理番号の順に並ばない（探しにくい）",
  ...
  (u"★氏名・住所で絞り込めない（管理番号だけ）",
   u'''    if(q && (o.mgmt + " " + (info.name || "") + " " + (info.addr || "")).toLowerCase().indexOf(q) < 0) continue;''',
   u'''    if(q && String(o.mgmt).toLowerCase().indexOf(q) < 0) continue;''',
   "smoke_v164_genba.js", "genba"),
  (u"★受付台帳の氏名・住所を出さない",
